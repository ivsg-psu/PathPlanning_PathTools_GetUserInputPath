...
% For zooming, keep track of handle limits by storeing initial limits
s = fcn_INTERNAL_fillAxisLimits(s, flag_isGeoPlot);
...
uiwait(figNum);    % block until uiresume or figure closed
	...
	function onClick(src,event)
		...
		% Check for clicks outside the previous axis. Ignore if this is the
		% case (this will happen if user clicks zoom +  or - buttons)
		if all(x>s.startXLim) || all(x<s.startXLim) || all(y>s.startYLim) || all(y<s.startYLim)
			return;
		end

		% Axis may have changed if user clicks zoom +, so check again
		s = fcn_INTERNAL_fillAxisLimits(s, flag_isGeoPlot);
		if all(x>s.startXLim) || all(x<s.startXLim) || all(y>s.startYLim) || all(y<s.startYLim)
			return;
		end
	...
	function onMouseMove(src,event)
		...
		% Make sure click was within the plot's prior boundaries
		if all(x>s.startXLim) || all(x<s.startXLim) || all(y>s.startYLim) || all(y<s.startYLim)
			s.active = false;
			setappdata(figNum,'HoldPanState',s);
			return;
		...
		% Don't pan if the plot hasn't started yet
	...
	function onButtonUp(src,event)
		...
		if isfield(s,'hasDragged') && s.hasDragged
			s.hasDragged = false; % Shut off dragging
			setappdata(figNum,'HoldPanState',s); % Save current state
		else
			...
			%%%%%%%%%%%%%%
			% Check for zoom event. If user hits zoom, the x or y value
			% will be outside the limits
			if all(x>s.startXLim) || all(x<s.startXLim) || all(y>s.startYLim) || all(y<s.startYLim)
				return;
			end
			...
			elseif strcmp(inputType,'onesidedsegment')
				...
				if all(isnan(pathXY),'all')
					% If nothing but NaN values, replace entire array with the first value
					pathXY = [x, y];
				else
					pathXY(end+1,:) = [x y];
				end
...
%% fcn_INTERNAL_findNearestPointIndex
...
end % Ends fcn_INTERNAL_findNearestPointIndex
...
%% fcn_INTERNAL_addPointToOneSidedSegmentPath
...
end % Ends fcn_INTERNAL_addPointToOneSidedSegmentPath
...
%% fcn_INTERNAL_fillAxisLimits
function s = fcn_INTERNAL_fillAxisLimits(s, flag_isGeoPlot)
if flag_isGeoPlot
...
end % Ends fcn_INTERNAL_fillAxisLimits